...
| Service | Internal Port | Host Port | Responsibility |
|---|---|---|---|
| `account-service` | 8001 | **8011** | Create/read accounts, cached balance reads |
| `ledger-service` | 8002 | **8012** | Read ledger entries, run the zero-sum and balance-reconciliation audit checks |
| `transaction-service` | 8003 | **8013** | The only service that writes to `transactions` and `ledger_entries` — owns the atomic transfer + idempotency logic |

Host ports are mapped to 8011/8012/8013 instead of 8001/8002/8003 to avoid colliding with ShopSphere containers that might still be running on this machine. The containers still listen on 8001/8002/8003 *inside* the Docker network — only the host-side mapping changed.
...
```bash
curl http://localhost:8011/accounts | python3 -m json.tool
```
...
```bash
curl -X POST http://localhost:8013/transactions/transfer \
